	...
	void CstoDlg::Fiil_list(CString zapros)
{
	...
	if(win.DoModal()==IDOK)
	{
		...
		while(pos)
		{
			...
			zapros.Format(_T("update sto SET marka_avto='%s',num_avto='%s',rik_vipusku='%s',problem='%s',zakluchenie='%s',cost='%s' WHERE FIO = '%s'"),win.m_avt,win.m_num,win.m_rik,win.m_prob,win.m_zakl,win.m_price,win.m_fio);
			db.ExecuteSQL(zapros);
